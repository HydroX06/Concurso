/* ═══════════════════════════════════════════════
   juego.js — Inversión Arriesgada (POO)
   ═══════════════════════════════════════════════ */
...
class Juego {
    constructor(){
        ...
        this.niveles              = this._construirNiveles()
...
/* ════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════ */
class Interfaz {
    static actualizarEtiquetaNivel(){
    ...
    static actualizarContadoresComodines(){
...
/* ════════════════════════════════════════════════
   CLASE: GestorPantallas
   Pantalla de fin de tramo y panel de compras
   ════════════════════════════════════════════════ */
class GestorPantallas {
    static mostrarPantallaTramo(){
        Interfaz.ocultarTodo()
        ...
        GestorPantallas.construirPanelCompras()
        Ruleta.dibujar(Ruleta.angulo)
        if(juego.ruletaActiva){ setTimeout(() => Ruleta.girar(), 600) }
    }

    static construirPanelCompras(){
        ...
        lista.forEach(c => {
            ...
            if(yaComprado){
                ...
                boton.onclick = () => GestorPantallas.comprarComodin(c.clave, coste)
            }
            contenedor.appendChild(boton)
        })
    }

    static comprarComodin(clave, coste){
        if(juego.comodinesCompradosEnNivel[clave]){ Interfaz.mostrarNotificacion("⚠ Ya compraste este comodín en este tramo", "error"); return }
        if(juego.dinero < coste){ Interfaz.mostrarNotificacion("⚠ CAPITAL INSUFICIENTE", "error"); return }
        juego.dinero -= coste
        juego.comodines[clave] = 1
        juego.comodinesCompradosEnNivel[clave] = true
        Interfaz.actualizarEstadisticas(); Interfaz.actualizarContadoresComodines()
        Interfaz.mostrarNotificacion("✓ Comodín recuperado", "success")
        GestorPantallas.construirPanelCompras()
    }
}

/* ════════════════════════════════════════════════
   CLASE: GestorComodines
   Lógica de cada comodín y potenciador
   ════════════════════════════════════════════════ */
class GestorComodines {
    static cincuentaCincuenta(){
        if(!juego.comodines.tieneUsos("cincuentaCincuenta") && !juego.modoComodinesInfinitos){
            Interfaz.mostrarNotificacion("⚠ Sin usos disponibles para este comodín", "error"); return
        }
    ...
    static cambiarPregunta(){
        ...
        if(disponibles.length === 0){ Interfaz.mostrarNotificacion("⚠ No hay más preguntas disponibles en este tramo", "error"); return }
        ...
        FlujoConcurso.mostrarPregunta()
        FlujoConcurso.iniciarTemporizador()
    }

    static comodinesInfinitos(){
        const coste = 300
        if(juego.comodinesInfinitosUsadoEsteTramo){ Interfaz.mostrarNotificacion("⚠ Comodines infinitos ya usado en este tramo", "error"); return }
        if(juego.dinero < coste){ Interfaz.mostrarNotificacion("⚠ CAPITAL INSUFICIENTE (necesitas " + coste + " €)", "error"); return }
        juego.dinero -= coste
        ...
        Interfaz.mostrarNotificacion("♾️ Comodines infinitos — solo esta pregunta (300 €)", "success")
    }

    static doblePuntos(){
        if(juego.doblePuntosUsadoEsteTramo){ Interfaz.mostrarNotificacion("⚠ Doble puntuación ya usado en este tramo", "error"); return }
        juego.doblePuntosUsadoEsteTramo = true
        juego.doblePuntosActivo = true
        Interfaz.mostrarNotificacion("✦ Doble puntos activados (1 pregunta)", "info")
    }

    static activarRuleta(){
        juego.ruletaActiva = true
        Interfaz.mostrarNotificacion("⟳ Modo ruleta — úsalo en fin de tramo", "info")
    }

    static reiniciarPotenciadores(){
        ...
        if(juego.modoComodinesInfinitos){
            ...
            Interfaz.actualizarContadoresComodines()
        }
    }
}
...
/* ════════════════════════════════════════════════
   CLASE: Ruleta
   Animación y lógica del canvas de la ruleta
   ════════════════════════════════════════════════ */
class Ruleta {
    static angulo  = 0
    static girando = false
    ...
    static dibujar(angulo){
        ...
        Ruleta.sectores.forEach((sector, i) => {
    ...
    static girar(){
        ...
        if(juego.ruletaUsadaEsteTramo){ Interfaz.mostrarNotificacion("⚠ Solo un giro de ruleta por fin de tramo", "error"); return }
        Ruleta.girando = true
        ...
        const tiempoInicio = performance.now(), anguloInicio = Ruleta.angulo
        ...
        function fotograma(ahora){
            ...
            Ruleta.angulo = anguloInicio + giroTotal*suavizado
            Ruleta.dibujar(Ruleta.angulo)
            if(t < 1){ requestAnimationFrame(fotograma); return }

            const porcion = (2*Math.PI)/Ruleta.sectores.length
            const normalizado = (((-Ruleta.angulo-Math.PI/2)%(2*Math.PI))+2*Math.PI)%(2*Math.PI)
            const indice = Math.floor(normalizado/porcion)%Ruleta.sectores.length
            const resultado = Ruleta.sectores[indice]
            ...
            if(texto.includes("€")){
                ...
                juego.dinero += valor; Interfaz.actualizarEstadisticas()
                Interfaz.mostrarNotificacion("🎲 Ruleta: "+texto, texto.startsWith("-") ? "error" : "success")
            } else if(texto.includes("vida")){
                const delta = texto.startsWith("+") ? 1 : -1
                juego.vidas = Math.max(0, juego.vidas + delta)
                Interfaz.actualizarEstadisticas()
                Interfaz.mostrarNotificacion("🎲 Ruleta: "+texto, delta < 0 ? "error" : "success")
                ...
                Interfaz.mostrarNotificacion("🎲 Ruleta: "+texto+" (próxima pregunta)", texto.startsWith("-") ? "error" : "success")
            }
        }
...
class FlujoConcurso {
    static comenzar(){
        ...
        Interfaz.actualizarContadoresComodines()
        Interfaz.actualizarEstadisticas()
        Interfaz.actualizarEtiquetaNivel()
        FlujoConcurso.siguientePregunta()
    }

    static siguientePregunta(){
        const listado = juego.preguntasNivelActual
        if(!listado || listado.length === 0){ FlujoConcurso.finJuego(); return }

        if(juego.indicePreguntaActual >= listado.length){
            clearInterval(juego.intervalo)
            GestorPantallas.mostrarPantallaTramo()
            return
        }
    ...
    static mostrarPregunta(){
        ...
        opciones.forEach((opcion, i) => {
            ...
            boton.onclick = () => FlujoConcurso.responder(i)
            contenedor.appendChild(boton)
        })
    }

    static responder(indice){
        ...
        if(pregunta.esCorrecta(indice)){
            ...
            setTimeout(() => {
                Interfaz.mostrarNotificacion("✓ CORRECTO — +" + ganancia + " €", "success")
                Interfaz.actualizarEstadisticas()
                GestorComodines.reiniciarPotenciadores()
                FlujoConcurso.siguientePregunta()
            }, 900)
        } else {
            ...
            setTimeout(() => {
                if(juego.escudoActivo){
                    Interfaz.mostrarNotificacion("🛡 ESCUDO ACTIVADO — Daño bloqueado", "shield")
                    ...
                    if(penalizacion === 2) Interfaz.mostrarNotificacion("✗ INCORRECTO — Doble penalización: -2 vidas", "error")
                }
                Interfaz.actualizarEstadisticas()
                if(juego.vidas <= 0){ clearInterval(juego.intervalo); setTimeout(FlujoConcurso.finJuego, 400); return }
                GestorComodines.reiniciarPotenciadores()
                FlujoConcurso.siguientePregunta()
            }, 900)
        }
    ...
    static iniciarTemporizador(){
        ...
        if(juego.bonusTiempoRuleta !== 0){
            ...
            Interfaz.mostrarNotificacion("⏱ " + (juego.bonusTiempoRuleta > 0 ? "+" : "") + juego.bonusTiempoRuleta + "s de la ruleta",
        ...
        juego.intervalo = setInterval(() => {
            ...
            if(juego.tiempo <= 0){
                clearInterval(juego.intervalo)
                juego.vidas--; Interfaz.actualizarEstadisticas()
                if(juego.vidas <= 0){ setTimeout(FlujoConcurso.finJuego, 400); return }
                GestorComodines.reiniciarPotenciadores()
                FlujoConcurso.siguientePregunta()
            }
        }, 1000)
    ...
    static continuarJuego(){
        ...
        if(juego.indiceNivelActual >= juego.ordenNiveles.length){ FlujoConcurso.mostrarVictoria(); return }
        juego.prepararNivel()
        Interfaz.actualizarEtiquetaNivel()
        FlujoConcurso.siguientePregunta()
    }
    ...
    static plantarse(){
        clearInterval(juego.intervalo); Interfaz.ocultarTodo()
...
/* ════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════ */
const comenzar          = () => FlujoConcurso.comenzar()
const continuarJuego    = () => FlujoConcurso.continuarJuego()
const plantarse         = () => FlujoConcurso.plantarse()
const reiniciarJuego    = () => FlujoConcurso.reiniciar()

const cincuentaCincuenta = () => GestorComodines.cincuentaCincuenta()
const cambiarPregunta    = () => GestorComodines.cambiarPregunta()
const tiempoExtra        = () => GestorComodines.tiempoExtra()
const dobleOportunidad   = () => GestorComodines.dobleOportunidad()
const comodinesInfinitos = () => GestorComodines.comodinesInfinitos()
const doblePuntos        = () => GestorComodines.doblePuntos()
const activarRuleta      = () => GestorComodines.activarRuleta()
const girarRuleta        = () => Ruleta.girar()
